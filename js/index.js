...
const BASE_GRID_SIZE = 8; // Base grid size for 2 players

// Define cannon firing directions based on position
const CANNON_DIRECTIONS = {
  ul: { hearts: "up", diamonds: "left" },
  ur: { hearts: "up", diamonds: "right" },
  ll: { clubs: "left", spades: "down" },
...
function calculateGridSize(numPlayers) {
  ...
  // Use logarithmic scaling with some adjustments for better progression
  // This formula gives us approximately:
  // 2 players -> 8
  // 3 players -> 11
  // 4 players -> 13
  // 5 players -> 14
  // 6 players -> 15
  // etc...
  const baseIncrease = 3; // Initial jump from 2 to 3 players
  const logBase = 1.5; // Controls how quickly the size increases
  return Math.floor(
    BASE_GRID_SIZE + baseIncrease + Math.log(numPlayers) / Math.log(logBase)
  );
}

// These will be updated whenever the player count changes
...
function isValidBoatPlacement(grid, x, y, gridSize) {
  // Boundary checking - ensure boat is fully within grid
  if (
    x < 0 ||
    x + BOAT_SIZE - 1 > gridSize + 1 ||
    y < 0 ||
    y + BOAT_SIZE - 1 > gridSize + 1
  )
    return false;
...
function placeBoats(numPlayers) {
  // The effective area where boats can be placed (accounting for boat size)
  // Add 1 to allow boats to touch the edge
  const effectiveGridSize = GRID_SIZE - BOAT_SIZE + 1;
  let grid = createGridArray(GRID_SIZE);
  let boatPositions = [];
  let attempts = 0;
  ...
  while (boatPositions.length < numPlayers && attempts < maxAttempts) {
    if (attempts > 0 && boatPositions.length === 0) {
      ...
      grid = createGridArray(GRID_SIZE);
    }

    const x = Math.floor(Math.random() * effectiveGridSize);
    const y = Math.floor(Math.random() * effectiveGridSize);

    if (isValidBoatPlacement(grid, x, y, GRID_SIZE)) {
...
function updatePlayerCount() {
  ...
  // Update grid size based on player count
  GRID_SIZE = calculateGridSize(count);
  SQUARE_SIZE = 100 / GRID_SIZE;

  const sqrElement = document.getElementById("sqr");
  if (sqrElement) {
    const boatLayout = placeBoats(count);
    if (boatLayout) {
      const { positions } = boatLayout;
      // Set the background grid size to match our GRID_SIZE
      sqrElement.style.backgroundSize = `${SQUARE_SIZE}vmin ${SQUARE_SIZE}vmin, ${SQUARE_SIZE}vmin ${SQUARE_SIZE}vmin, 20% 20%`;
      ...
      if (gameGrid) {
        ...
        positions.forEach((pos, index) => {
          if (index < players.length) {
            ...
            // Calculate position in grid units (each grid square is squareSize vmin)
            const left = pos.x * SQUARE_SIZE + "vmin";
            const top = pos.y * SQUARE_SIZE + "vmin";
            const width = BOAT_SIZE * SQUARE_SIZE + "vmin";
            const height = BOAT_SIZE * SQUARE_SIZE + "vmin";
...
let playerHand = [];
let selectedCards = [];
let confirmedCards = new Map(); // Maps player names to their confirmed cards
let playerReadyForNextRound = new Map(); // Track which players have confirmed cards
const TURN_TIMER = 45000; // 45 seconds for card selection
let turnTimer = null;

// Set up networking callbacks for the card system
if (typeof networkManager !== "undefined") {
  networkManager.setCallbacks({
...
function getMovementDirection(suit, position) {
  const cannonDirection = CANNON_DIRECTIONS[position]?.[suit];
  if (!cannonDirection) return null;

  // Return opposite direction for movement
...
window.moveBoat = function (playerName, direction) {
  ...
  switch (direction) {
    case "up":
      boatDiv.style.top = currentTop - moveAmount + "vmin";
      break;
    case "down":
      boatDiv.style.top = currentTop + moveAmount + "vmin";
      break;
    case "left":
      boatDiv.style.left = currentLeft - moveAmount + "vmin";
      break;
    case "right":
      boatDiv.style.left = currentLeft + moveAmount + "vmin";
      break;
...
function startTurnTimer() {
  ...
  const updateTimer = () => {
    const remaining = TURN_TIMER - (Date.now() - startTime);
    if (remaining > 0) {
...
function processTurn() {
  if (isHost) {
    ...
    // Broadcast moves to all clients
    if (networkManager?.getSocket()) {
