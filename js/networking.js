...
let onGameStarting;
let onRoomClosed;
let onPlayerCardsConfirmed;
let onAllCardsConfirmed;
...
function setupSocketEventHandlers() {
  ...
  // Handle card confirmation events
  socket.on("player-cards-confirmed", (data) => {
    if (onPlayerCardsConfirmed) {
      onPlayerCardsConfirmed(data.playerName, data.cards);
    }
  });

  socket.on("all-cards-confirmed", () => {
    if (onAllCardsConfirmed) {
      onAllCardsConfirmed();
    }
  });

  socket.on("execute-moves", (data) => {
    // Execute the moves on all clients
    data.moves.forEach((move) => {
      if (typeof window.moveBoat === "function") {
        window.moveBoat(move.playerName, move.direction);
      }
    });
  });
}
