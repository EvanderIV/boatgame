...
io.on("connection", (client) => {
  ...
  // Handle card confirmations from players
  client.on("cards-confirmed", (data) => {
    if (!roomCode) return;
    ...
    // Store the confirmed cards
    room.confirmedCards.set(playerData.name, data.cards);

    // Notify host about the card confirmation
    io.to(room.hostId).emit("player-cards-confirmed", {
      playerName: playerData.name,
      cards: data.cards,
    });

    // Check if all players have confirmed
    if (room.confirmedCards.size === room.players.size) {
      // All players have confirmed their cards
      io.to(room.hostId).emit("all-cards-confirmed");
    }
  });
